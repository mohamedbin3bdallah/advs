...
if(isset($_SESSION['userid']))
{
	...
	include('develops/pay-front.php');
?>
<!DOCTYPE html>
<html lang="en">
  <head>
        ...
        <div class="row">         
          <div class="col-md-12">
            <div class="breadcrumb-wrapper">
              <h2 class="page-title"><?php language('pay'); ?></h2>
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="col-sm-6 col-sm-offset-3 col-md-4 col-md-offset-4">
            <div class="page-login-form box">
			  ...
			  <p style="text-align:center;">
				<?php
					if(isset($_GET['message']) && $_GET['message'] == 'm1') echo '<div style="color:green;">تمت العملية بنجاح</div>';
					elseif(isset($_GET['message']) && $_GET['message'] == 'm2') echo '<div style="color:red;">حدث خطأ ما</div>';
					elseif(isset($_GET['message']) && $_GET['message'] == 'm3') echo '<div style="color:red;">البريد الاكتروني للباي بال غير متاح</div>';
				?>
			</p>
              <form role="form" class="login-form" action="#" method="post">
                  ...
                  <div class="input-icon">
                    ...
                    <select name="planid" class="form-control" style="height:51px; padding:0px 22px;" required>
						<?php for($pl=0;$pl<count($plans);$pl++) { ?>
							<option value="<?php echo $plans[$pl]['id'].'|'.$plans[$pl]['fees']; ?>" <?php if(isset($_GET['id']) && $_GET['id'] == $plans[$pl]['id']) echo 'selected'; ?>><?php echo $plans[$pl]['title'].' - '.$plans[$pl]['fees'].' جنيه / '; language('month'); ?></option>
						<?php } ?>
					</select>
                  </div>
                </div> 
                <div class="form-group">
                  <div class="input-icon">
					<label><?php language('paymethod'); ?></label>
                    <select name="paymethod" id="paymethod" class="form-control" style="height:51px; padding:0px 22px;" required>
						<option value="bank"><?php language('bank'); ?></option>
						<option value="paypal"><?php language('paypal'); ?></option>
					</select>
				...
				<div class="form-group">
                  <div class="input-icon">
					<label><?php language('adsnumber'); ?></label>
                ...
                <button type="submit" class="btn btn-common log-btn" name="submit" id="submit"><?php language('pay'); ?></button>
              </form>
            </div>
          </div>
        </div>
