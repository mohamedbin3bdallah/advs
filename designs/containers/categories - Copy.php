		...
		<div class="col-sm-8 col-sm-offset-2">
			<form class="form-horizontal" action="admin.php?c=<?php echo $_GET['c']; ?>" method="POST" enctype="multipart/form-data" autocomplete="off">
				...
				<div class="form-group">
					<label for="type" class="col-sm-2 <?php if($lang == 'ar') echo 'col-sm-push-10'; ?> control-label"><?php language('type'); ?></label>
					<div class="col-sm-10 <?php if($lang == 'ar') echo 'col-sm-pull-2'; ?>">
						<select class="form-control" name="type" id="type">
							<option value="doctor" <?php if(isset($row['type']) && $row['type'] == 'doctor') echo 'selected';?>><?php language('doctor'); ?></option>
							<option value="company" <?php if(isset($row['type']) && $row['type'] == 'company') echo 'selected';?>><?php language('company'); ?></option>
						</select>
					</div>
				</div>
				<div class="form-group">
			...
			<table class="table table-bordered table-striped" style="text-align: center;">
				...
				<?php	for($i=0;$i<sizeof($data);$i++)	{	?>
					<tr id="tr-<?php echo $data[$i]['id'];?>">
						...
						<td><?php language($data[$i]['type']); ?></td>
							...
							<div id="del-<?php echo $data[$i]['id'];?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
								<div class="modal-dialog modal-sm">
									<div class="modal-content">
										...
										<center>
											<button class="btn btn-danger" onclick="mydelete('<?php echo $data[$i]['id'];?>','categoryid','categories','sitepages')" data-dismiss="modal"><?php language('agree'); ?></button>
