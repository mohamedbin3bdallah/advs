<?php
$data = getAllDataFromTable($select,'features',$where,$limit);
if(count($data) < 6 || isset($_GET['id']))
{
?>	
	<div class="row">	
			...
			<form class="form-horizontal" action="admin.php?c=<?php echo $_GET['c']; ?>" method="POST" enctype="multipart/form-data" autocomplete="off">
				...
				<div class="form-group">
					...
					<div class="col-sm-10 <?php if($lang == 'ar') echo 'col-sm-pull-2'; ?>">
						<input type="text" class="form-control" name="title" id="title" value="<?php if(isset($row['title'])) echo $row['title']; ?>" maxlength="150" placeholder="<?php language("title"); ?>" autocomplete="off" />
					</div>
				</div>
				<div class="form-group">
					<label for="description" class="col-sm-2 <?php if($lang == 'ar') echo 'col-sm-push-10'; ?> control-label"><?php language('description'); ?></label>
					<div class="col-sm-10 <?php if($lang == 'ar') echo 'col-sm-pull-2'; ?>">
						<textarea class="form-control" name="description" id="description" placeholder="<?php language("description"); ?>" rows="5" required><?php if(isset($row['description'])) echo $row['description']; ?></textarea>
					</div>
				</div>
				<div class="form-group">
					<label for="picture" class="col-sm-2 <?php if($lang == 'ar') echo 'col-sm-push-10'; ?> control-label"><?php language('picture'); ?></label>
					<div class="col-sm-10 <?php if($lang == 'ar') echo 'col-sm-pull-2'; ?>">
						<select type="text" class="form-control" name="picture" id="picture" required>
							<option value="images/feature-1.png"><?php language('person'); ?></option>
							<option value="images/feature-2.png"><?php language('map'); ?></option>
							<option value="images/feature-3.png"><?php language('headphone'); ?></option>
							<option value="images/feature-4.png"><?php language('clickbutton'); ?></option>
							<option value="images/feature-5.png"><?php language('24hourservice'); ?></option>
							<option value="images/feature-6.png"><?php language('settings'); ?></option>
						</select>
					</div>
				</div>
				<div class="form-group">
			...
			<table class="table table-bordered table-striped" style="text-align: center;">
				...
				<?php	for($i=0;$i<sizeof($data);$i++)	{	?>
					<tr id="tr-<?php echo $data[$i]['id'];?>">
						...
						<td><img src="<?php echo $data[$i]['picture']; ?>" width="30%"></td>
							...
							<div id="del-<?php echo $data[$i]['id'];?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
								<div class="modal-dialog modal-sm">
									<div class="modal-content">
										...
										<center>
											<button class="btn btn-danger" onclick="mydelete('<?php echo $data[$i]['id'];?>','features')" data-dismiss="modal"><?php language('agree'); ?></button>
