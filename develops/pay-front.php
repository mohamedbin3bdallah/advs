...
$thispage = getRowFromTable('*','pages','where page like "pay.php"','');
...
if(isset($_POST['submit']))
{
	...
	$arr = explode('|', $_POST['planid']);
		...
		{
			if(insertRow('orders', array('userid'=>$_SESSION['userid'], 'planid'=>$arr[0], 'number'=>$_POST['number'], 'renumber'=>$_POST['number'], 'total'=>$_POST['number']*$arr[1], 'time'=> time())))	header('Location: pay.php?message=m1');
			else header('Location: pay.php?message=m2');
		}
		elseif($_POST['paymethod'] == 'paypal')
		{
			if($system['paypal'] != '')
			{
				...
				var no_shipping = 1;
				var handling = 0;
				var cancel_return = "<?php echo 'http://'.$_SERVER['HTTP_HOST'].'/pay.php?message=m2'; ?>";
				var success_return = "<?php echo 'http://'.$_SERVER['HTTP_HOST'].'/pay.php?oid='.$orderid; ?>";
			...
			else header('Location: pay.php?message=m3');
		}
}
elseif(isset($_GET['oid']) && abs((int)($_GET['oid'])) > 0)
{
	...
	{
		if(update('orders','set delivered = 1 , dtime = '.time(),'where id = '.$_GET['oid'].' and userid = '.$_SESSION['userid'])) header('Location: pay.php?message=m1');
		else header('Location: pay.php?message=m2');
	}
	else header('Location: pay.php?message=m2');
}
